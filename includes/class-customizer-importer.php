 ...
 * Customizer Importer
 */
class Codegear_Starter_Customizer_Importer {
	/**
	 * Import customizer.
	 *
	...
	public static function import_customizer_options( $data ) {
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'codegear-starter' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'codegear-starter' )
			);
		}

		// Import images.
		if ( apply_filters( 'codegear_customizer_import_images', true ) ) {
		...
		if ( isset( $data['options'] ) ) {
			...
			if ( ! class_exists( 'Codegear_Customizer_Option' ) ) {
				require_once CODEGEAR_STARTER_PATH . 'includes/class-customizer-option.php';
			}

			foreach ( $data['options'] as $option_key => $option_value ) {
				$option = new Codegear_Customizer_Option( $wp_customize, $option_key, array(
					'default'    => '',
					'type'       => 'option',
		...
		$use_wp_customize_save_hooks = apply_filters( 'codegear_enable_wp_customize_save_hooks', false );
