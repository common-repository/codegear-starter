...
class Codegear_Starter_Import {
	...
	public function init() {
		...
		add_action( 'wp_ajax_starter_import_plugin', array( $this, 'ajax_import_plugin' ) );
		add_action( 'wp_ajax_starter_import_contents', array( $this, 'ajax_import_contents' ) );
		add_action( 'wp_ajax_starter_import_customizer', array( $this, 'ajax_import_customizer' ) );
		add_action( 'wp_ajax_starter_import_widgets', array( $this, 'ajax_import_widgets' ) );
		add_action( 'wp_ajax_starter_import_options', array( $this, 'ajax_import_options' ) );
		add_action( 'wp_ajax_starter_import_finish', array( $this, 'ajax_import_finish' ) );
	}
	...
	public function ajax_import_plugin() {
		...
		if ( isset( $_POST['demo_id'] ) ) {
			set_transient( 'starter_importer_demo_id', sanitize_key( wp_unslash( $_POST['demo_id'] )) );
		}
		 ...
		 * Action hook.
		 */
		do_action( 'starter_import_plugin', $plugin_slug, $plugin_path );
	...
	public function ajax_import_contents() {
		...
		$xml_file_path = get_transient( 'starter_importer_data' );
		...
		if ( ! $xml_file_path ) {
			...
			set_transient( 'starter_importer_data', $xml_file_path, HOUR_IN_SECONDS );
		}
		...
		$logger_options = apply_filters( 'starter_logger_options', array(
		 ...
		 * Action hook.
		 */
		do_action( 'starter_import_contents' );
	...
	public function ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'starter_time_for_one_ajax_call', 22 ) ) {
			$response = array(
				'success' => true,
				'status'  => 'newAJAX',
	...
	public function ajax_import_customizer() {
		...
		do_action( 'starter_import_customizer', $data );
	...
	public function ajax_import_widgets() {
		...
		do_action( 'starter_import_widgets', $sidebar_widgets, $widget_instances );
	...
	public function ajax_import_options() {
		 ...
		 * Action hook.
		 */
		do_action( 'starter_import_options', $array );
	...
	public function ajax_import_finish() {
		 ...
		 * Action hook.
		 */
		do_action( 'starter_finish_import' );
